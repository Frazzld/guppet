...
function GupPet_Interface_BuildMacro( self, option )
	...
	--[[	PRIEST 										]]--
	--------------------------------------------------------
	if GUPPET_TEMPCLASSDATA.Type == "PRIEST" and option ~= "FLY"  then
	
		if  GUPPET_TEMPCLASSDATA["Levitate"]["Usable"] then
			self:SetAttribute("alt-macrotext1", "/use "..GUPPET_TEMPCLASSDATA["Levitate"]["Name"] );
		end
	
		if GUPPET_OPTIONS["Class"]["LevitateOnFalling"] and GUPPET_TEMPCLASSDATA["Levitate"]["Usable"]  and IsFalling() and not (IsMounted() or UnitInVehicle("player")) then
	...
	if GUPPET_TEMPCLASSDATA.Type == "HUNTER" and  option ~= "FLY"  then
		...
		--Cheetah on moving
		if GUPPET_TEMPCLASSDATA["Cheetah"]["Usable"] and  GetUnitSpeed("player") > 0  and not(IsMounted()) and  GUPPET_OPTIONS["Class"]["CheetahOnWalking"]  then
	...
	if GUPPET_TEMPCLASSDATA.Type == "DRUID"   then
		...
		if option == "FLY" then
			...
			if (GUPPET_OPTIONS["Class"]["FlyForm"] or IsFalling() ) and GUPPET_TEMPCLASSDATA["Fly"]["Usable"] then			
				self:SetAttribute("macrotext"		, "/use "..GUPPET_TEMPCLASSDATA["Fly"]["Name"] );
				self:SetAttribute("shift-macrotext1", "/use "..GUPPET_TEMPCLASSDATA["Fly"]["Name"] );
				self:SetAttribute("alt-macrotext1"	, "/use "..GUPPET_TEMPCLASSDATA["Fly"]["Name"] );
			elseif (GUPPET_OPTIONS["Class"]["FlyForm"] or IsFalling() ) and GupPet_CanFly() and GUPPET_TEMPCLASSDATA["Travel"]["Usable"] then
				self:SetAttribute("macrotext"		, "/use "..GUPPET_TEMPCLASSDATA["Travel"]["Name"] );
				self:SetAttribute("shift-macrotext1", "/use "..GUPPET_TEMPCLASSDATA["Travel"]["Name"] );
				self:SetAttribute("alt-macrotext1"	, "/use "..GUPPET_TEMPCLASSDATA["Travel"]["Name"] );
		...
		elseif option == "GROUND" then
			...
			if GupPet_CanFly() and IsOutdoors() then 
				...
				if (GUPPET_OPTIONS["Class"]["FlyForm"] or IsFalling() ) and GUPPET_TEMPCLASSDATA["Fly"]["Usable"] then			
					self:SetAttribute("macrotext"		, "/use "..GUPPET_TEMPCLASSDATA["Fly"]["Name"] )
				elseif (GUPPET_OPTIONS["Class"]["FlyForm"] or IsFalling() ) and GUPPET_TEMPCLASSDATA["Travel"]["Usable"] then			
					self:SetAttribute("macrotext"		, "/use "..GUPPET_TEMPCLASSDATA["Travel"]["Name"] )
				elseif (GetShapeshiftFormID() == MOONKIN_FORM) then
...
function GupPet_Interface_IngameButtons()
	...
	if GUPPET_OPTIONS.IngameButton.MountButton == "Auto" and GUPPET_Z["SUPPORT"] and ( GUPPET_SAVEDDATA[ "Fly" ]["Total"] > 0  or (GUPPET_TEMPCLASSDATA.Type == "DRUID" and GUPPET_TEMPCLASSDATA["Travel"]["Usable"]) ) then
