...
function GupPet_OnEvent(self, event, ...) --self, event, ...
	...
	if event == "PLAYER_ENTERING_WORLD" then
		...
		PrefixRegistered = RegisterAddonMessagePrefix("GupPetVersion");
		if IsInGuild() then
	...
	elseif event == "LEARNED_SPELL_IN_TAB" or event == "GLYPH_REMOVED" then 
	...
	elseif event == "CHAT_MSG_ADDON" then
		...
		if arg1 == "GupPetVersion" then -- Listen for new versions
			...
			if GupPet_ToNumber(n_arg2) > GupPet_ToNumber(GetAddOnMetadata("GupPet", "Version")) and GupPet_ToNumber(n_arg2) > GupPet_ToNumber(GUPPET_OPTIONS.NewVersion)   then	 
...
----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------
function GupPet_SendMyVersion( To )	--GUILD
	if PrefixRegistered then	
		local MyVersion = GetAddOnMetadata("GupPet", "Version") ;
		--Send latest known version of the addon.
		if GupPet_ToNumber(MyVersion) < GupPet_ToNumber(GUPPET_OPTIONS.NewVersion) then
...
function GupPet_UpdateClassDataList()
	...
	elseif GUPPET_TEMPCLASSDATA.Type == "DRUID" then
		...
		GUPPET_TEMPCLASSDATA["Fly"] = GupPet_IsInBook( GUPPET_CLASSDATA.DRUID.Fly ) ;
...
function GupPet_ProfessionUpdate()
...
function GupPet_ToNumber(version)
	local PreNumVersion = gsub(version,"%D","")
	local NumVersion = tonumber(PreNumVersion)
	return NumVersion
end