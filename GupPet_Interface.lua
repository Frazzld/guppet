...
function GupPet_Interface_Option()
	...
	if GUPPET_INTERFACE_OPTIONFRAME.Init == 0 then
		...
		if GUPPET_TEMPRACEDATA.Type == "Worgen" then
			...
			GUPPET_INTERFACE_OPTIONFRAME.RaceFrame:SetPoint("RIGHT", GupPet_RaceFrame:GetParent():GetName(), "RIGHT", -10, 0);
		...
		GUPPET_INTERFACE_OPTIONFRAME.SlashCommands:SetText("<html><body><p>"..
															...
															" </p></body></html>"    ) ;
		GUPPET_INTERFACE_OPTIONFRAME.SlashCommands:SetFontObject(GupPet_FontText);
...
function GupPet_Interface_UpdateLocalFrame( Data ,  MountType )
  ...
  local t_i = 0 ;
	...
	for i = 1 , GUPPET_SAVEDDATA[ MountType ]["Total"] do
		...
		if MountType == "Companion" then _G[MountType..i]:SetScript("OnEnter", function(self, button, down)	GupPet_PreviewModel:SetCreature( GUPPET_SAVEDDATA[MountType][ i ]["CreatureID"]  )	GupPet_Preview:Show() end);
			else _G[MountType..i]:SetScript("OnEnter", function(self, button, down)	GupPet_PreviewModel:SetDisplayInfo( GUPPET_SAVEDDATA[MountType][ i ]["CreatureID"]  )	GupPet_Preview:Show() end);
		end
  ...
  local t_i = 0 ;
	...
	for i = 1 , math.ceil(GUPPET_SAVEDDATA[ MountType ]["Total"]/2) do
		...
		if GUPPET_OPTIONS.Mode == "Expert" then
			...
			if (GUPPET_SAVEDDATA[MountType][ i ]["Weight"][ _G[GUPPET_INTERFACE_LOCATIONFRAME.Selected].Location ] > 0) then
				tempCheck = true;
			end
			_G[MountType..i]:SetChecked(tempCheck);
			_G[MountType..i]:SetScript("OnClick", function (self)   
					if ( IsModifiedClick() ) then   
	...
	for i = 1 , GUPPET_SAVEDDATA[ MountType ]["Total"]- math.ceil(GUPPET_SAVEDDATA[ MountType ]["Total"]/2) do
		...
		if GUPPET_OPTIONS.Mode == "Expert" then
			...
			local tempCheck = false;
			if  (GUPPET_SAVEDDATA[MountType][ ii ]["Weight"][ _G[GUPPET_INTERFACE_LOCATIONFRAME.Selected].Location ]  > 0) then
				tempCheck = true;
			end
			_G[MountType..ii]:SetChecked(tempCheck);
		...
		if MountType == "Companion" then _G[MountType..ii ]:SetScript("OnEnter", function(self, button, down)	GupPet_PreviewModel:SetCreature( GUPPET_SAVEDDATA[MountType][ ii ]["CreatureID"]  )	GupPet_Preview:Show() end);
			else _G[MountType..ii ]:SetScript("OnEnter", function(self, button, down)	GupPet_PreviewModel:SetDisplayInfo( GUPPET_SAVEDDATA[MountType][ ii ]["CreatureID"]  )	GupPet_Preview:Show() end);
		end
